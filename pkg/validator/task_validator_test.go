...
`
const taskWithInvalidImageRefV1 = `
apiVersion: tekton.dev/v1
kind: Task
metadata:
  name: invalid-image-refs
...
spec:
  ...
  - name: s2
    ...
    script: |
      ...
      print("""$(params.secret)""")
`
const taskWithScriptUsingParamsV1 = `
apiVersion: tekton.dev/v1
kind: Task
metadata:
  name: invalid-env-or-envfrom
...
func TestTaskValidator_ValidImageRefV1(t *testing.T) {
	r := strings.NewReader(taskWithValidImageRefV1)
	parser := parser.ForReader(r)
...
func TestTaskValidator_InvalidImageRef(t *testing.T) {
	...
	assert.Equal(t, `Step "s7" uses image "abc.io/fedora@sha256:deadb33fdeadb33fdeadb33fdeadb33fdeadb33fdeadb33fdeadb33fdeadb33f"; consider using an image tagged with specific version along with digest eg. abc.io/img:v1@sha256:abcde`, result.Lints[8].Message)

	// image with digest and latest tag
	assert.Equal(t, Warning, lints[9].Kind)
	assert.Equal(t, `Step "s8" uses image "abc.io/fedora:latest@sha256:deadb33fdeadb33fdeadb33fdeadb33fdeadb33fdeadb33fdeadb33fdeadb33f"; consider using an image tagged with specific version along with digest eg. abc.io/img:v1@sha256:abcde`, result.Lints[9].Message)
...
func TestTaskValidator_InvalidImageRefV1(t *testing.T) {
	r := strings.NewReader(taskWithInvalidImageRefV1)
	parser := parser.ForReader(r)
...
func TestTaskValidator_ValidEnvFromSecretV1(t *testing.T) {
	r := strings.NewReader(taskWithEnvFromSecretV1)
	parser := parser.ForReader(r)
...
func TestTaskValidator_ScriptUsingParamsV1(t *testing.T) {
	r := strings.NewReader(taskWithScriptUsingParamsV1)
	parser := parser.ForReader(r)
